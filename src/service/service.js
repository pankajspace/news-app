...
// const API_KEY = "d68d670f78734f3da20869b229b259a3";
const API_KEY = "0735d324ba874becb7beb8e73fea20f1";
...
export const getNews = async (
  ...
  const params = `?page=${pageNumber}&pageSize=${pageSize}&q=${searchTerm}&apiKey=${key}`;
  let result = { status: "LOADING", data: null };

  try {
    let response = await fetch(`${base}${params}`);
    result = { status: "SUCCESS", data: await response.json() };
  } catch (error) {
    result = { status: "ERROR", data: error };
  }

  // return { status: "SUCCESS", data: await dummyResponse };
